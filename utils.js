...
const updateUserTokens = async (filePath, username, fieldToUpdate, newObject) => {
  try {
    ...
    if (index !== -1) {
      ...
      if (!Object.prototype.hasOwnProperty.call(jsonData[index], fieldToUpdate)) {
    ...
    if (await fs.promises.access(filename).then(() => true).catch(() => false)) {
      ...
      if (index !== -1) {
        ...
        for (const key in newJsonObject) {
          if (Object.prototype.hasOwnProperty.call(newJsonObject, key) && newJsonObject[key] && !Object.prototype.hasOwnProperty.call(existingJsonArray[index], key)) {
